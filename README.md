...
**Categorías del producto (M:N — un producto puede estar en varias a la vez):** el formulario tiene un grid de checkboxes (uno por categoría, poblado con `GET /api/admin/categorias`) en vez de un selector de una sola — junto con un botón "+ Nueva categoría…" que abre un mini-formulario inline (nombre + un emoji como ícono, sin selector de imagen), llama `POST /api/admin/categorias` y deja la categoría nueva ya marcada, sin salir del modal ni perder lo que ya estaba tildado. Al guardar, `PUT/POST /api/admin/productos` sincroniza `producto_categorias` (borra todas las relaciones del producto e inserta las marcadas actuales — más simple que un diff, y no hay problema de performance para este volumen). `productos.categoria_id` queda **por compatibilidad**, no como fuente de verdad: se sincroniza sola con la primera categoría marcada (o `null` si no se marcó ninguna), sin `COALESCE` — mismo motivo que `regiones_disponibles`: desmarcar todo tiene que poder dejarlo en `null`, no quedarse pegado en la última.
...
- **`catalogo.html`** — grid de productos con filtro por categoría (chips + `?categoria=`, contra `producto_categorias`/`categorias.slug` reales — nunca datos de ejemplo) y buscador (`?q=`). `GET /api/productos?categoria=X` matchea si CUALQUIERA de las categorías del producto es `X` (`EXISTS` contra `producto_categorias`, no una columna única). Las tarjetas (acá, en el home y en la ficha de producto) muestran un badge por cada categoría del producto, no solo una.
...
- **`GET /api/admin/reportes/pedidos.csv?desde=&hasta=&proveedor_id=&organizacion_id=`** — descarga un CSV (una fila por pedido, con BOM UTF-8 para que Excel en Windows no rompa tildes/ñ) con columnas `fecha, codigo_pedido, cliente_nombre, cliente_email, producto, cantidad, organizacion, proveedor, monto_proveedor, monto_comision_afiliado, monto_comision_huayca, monto_total, estado_pago, estado_despacho, codigo_descuento` (esta última es `pedidos.codigo_descuento_usado`, vacía si el pedido no usó ningún código — va al final a propósito, para no correr la posición de las columnas que ya existían). A diferencia del resumen, el CSV **no** filtra por `estado_pago` — trae el registro completo del período (incluye pendientes y rechazados) porque es lo que Cristian usa para su propia contabilidad, donde la completitud importa más que "solo lo aprobado". El nombre del archivo es descriptivo (`huayca-pedidos-2026-08-01-a-2026-08-31.csv`) y se descarga con un `fetch` autenticado (no un link plano, porque la ruta exige el Bearer token) que arma un blob y dispara la descarga a mano.
